...
class PervObmTV1:
    ...
    def perv_obm_tv1(self, calc_vol: float = 0.0) -> None:
        if calc_vol < 3.67:
            ...
            self.cli_log.lev_info(f"перв. обм. 1: на вход получили: {calc_vol:.2f}, "
                                  f"< 3.67: отключение всех реле первичной обмотки", "orange")
            ...
            self.cli_log.lev_info(f"перв. обм. 1: на вход получили: {calc_vol:.2f}, 3.67 <= < 3.89: KL38 == True:",
                                  "orange")
            self.conn_opc.ctrl_relay('KL38', True)
        elif 3.89 <= calc_vol < 4.11:
            self.logger.debug(f"перв. обм. 2: на вход получили: {calc_vol:.2f}, 3.89 <= < 4.11: KL39 == True")
            self.cli_log.lev_info(f"перв. обм. 2: на вход получили: {calc_vol:.2f}, 3.89 <= < 4.11: KL39 == True:",
                                  "orange")
            self.conn_opc.ctrl_relay('KL39', True)
        elif 4.11 <= calc_vol < 4.35:
            self.logger.debug(f"перв. обм. 3: на вход получили: {calc_vol:.2f}, 4.11 <= < 4.35: KL40 == True")
            self.cli_log.lev_info(f"перв. обм. 3: на вход получили: {calc_vol:.2f}, 4.11 <= < 4.35: KL40 == True",
                                  "orange")
            self.conn_opc.ctrl_relay('KL40', True)
        elif 4.35 <= calc_vol < 4.57:
            self.logger.debug(f"перв. обм. 4: на вход получили: {calc_vol:.2f}, 4.35 <= < 4.57: KL41 == True")
            self.cli_log.lev_info(f"перв. обм. 4: на вход получили: {calc_vol:.2f}, 4.35 <= < 4.57: KL41 == True",
                                  "orange")
            self.conn_opc.ctrl_relay('KL41', True)
        elif 4.57 <= calc_vol < 4.77:
            self.logger.debug(f"перв. обм. 5: на вход получили: {calc_vol:.2f}, 4.57 <= < 4.77: KL42 == True")
            self.cli_log.lev_info(f"перв. обм. 5: на вход получили: {calc_vol:.2f}, 4.57 <= < 4.77: KL42 == True",
                                  "orange")
            self.conn_opc.ctrl_relay('KL42', True)
        elif 4.77 <= calc_vol < 5.03:
            self.logger.debug(f"перв. обм. 6: на вход получили: {calc_vol:.2f}, 4.77 <= < 5.03: KL43 == True")
            self.cli_log.lev_info(f"перв. обм. 6: на вход получили: {calc_vol:.2f}, 4.77 <= < 5.03: KL43 == True",
                                  "orange")
            ...
            self.cli_log.lev_info(f"перв. обм. 7: на вход получили: {calc_vol:.2f}, 5.03 <= < 5.25: KL44 == True",
                                  "orange")
...
class VtorObmTV1:
    ...
    def vtor_obm_tv1(self, calc_vol: float = 0.0) -> None:
        ...
        if calc_vol < 3.66:
            ...
            self.cli_log.lev_info(f"втор. обм. 1: на вход получили: {calc_vol:.2f}, "
                                  f"отключение всех реле втор. обмотки", "orange")
            ...
            self.cli_log.lev_info(f"втор. обм. 2: на вход получили: {calc_vol:.2f}, 3.67 <= <= 5.96: KL59",
                                  "orange")
            self.conn_opc.ctrl_relay('KL59', True)
        elif 5.97 <= calc_vol <= 9.64:
            self.logger.debug(f"втор. обм. 3: на вход получили: {calc_vol:.2f}, 5.97 <= <= 9.64: KL58")
            self.cli_log.lev_info(f"втор. обм. 3: на вход получили: {calc_vol:.2f}, 5.97 <= <= 9.64: KL58",
                                  "orange")
            self.conn_opc.ctrl_relay('KL58', True)
        elif 9.65 <= calc_vol <= 15.15:
            self.logger.debug(f"втор. обм. 4: на вход получили: {calc_vol:.2f}, 9.65 <= <= 15.15: KL57")
            self.cli_log.lev_info(f"втор. обм. 4: на вход получили: {calc_vol:.2f}, 9.65 <= <= 15.15: KL57",
                                  "orange")
            self.conn_opc.ctrl_relay('KL57', True)
        elif 15.16 <= calc_vol <= 23.87:
            self.logger.debug(f"втор. обм. 5: на вход получили: {calc_vol:.2f}, 15.16 <= <= 23.87: KL56")
            self.cli_log.lev_info(f"втор. обм. 5: на вход получили: {calc_vol:.2f}, 15.16 <= <= 23.87: KL56",
                                  "orange")
            self.conn_opc.ctrl_relay('KL56', True)
        elif 23.88 <= calc_vol <= 37.65:
            self.logger.debug(f"втор. обм. 6: на вход получили: {calc_vol:.2f}, 23.88 <= <= 37.65: KL55")
            self.cli_log.lev_info(f"втор. обм. 6: на вход получили: {calc_vol:.2f}, 23.88 <= <= 37.65: KL55",
                                  "orange")
            self.conn_opc.ctrl_relay('KL55', True)
        elif 37.66 <= calc_vol <= 59.7:
            self.logger.debug(f"втор. обм. 7: на вход получили: {calc_vol:.2f}, 37.66 <= <= 59.7: KL54")
            self.cli_log.lev_info(f"втор. обм. 7: на вход получили: {calc_vol:.2f}, 37.66 <= <= 59.7: KL54",
                                  "orange")
