...
class TestBMZ2:
    ...
    def __init__(self):
        ...
        self.list_ust_volt: tuple[float, ...] = (34.2, 42.2, 50.2, 58.3, 66.3, 74.3, 82.4, 90.4, 98.5, 106.5, 114.5)
    ...
    def delta_t(self) -> None:
        for qw in range(4):
            self.calc_delta_t, self.inp_01, self.inp_02, \
                self.inp_05, self.inp_06 = self.conn_opc.ctrl_ai_code_v0(code=104)
            self.logger.info(f'попытка {qw}, дельта t \t {self.calc_delta_t:.1f}')
            self.cli_log.lev_info(f'попытка {qw}, дельта t \t {self.calc_delta_t:.1f}', "orange")
