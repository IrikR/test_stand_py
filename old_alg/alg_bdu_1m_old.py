...
class TestBDU1M(object):
    ...
    def st_test_1_bdu_1m(self) -> bool:
        ...
        self.fault.debug_msg(f'{in_a2 = }', 'blue')
        if in_a2 is False:
            pass
        else:
            self.fault.debug_msg('тест 1 положение выходов не соответствует', 'red')
            self.mysql_conn.mysql_ins_result("неисправен", '1')
            if in_a1 is True:
                self.mysql_conn.mysql_error(198)
            elif in_a2 is True:
                self.mysql_conn.mysql_error(199)
            return False
        self.fault.debug_msg('тест 1 положение выходов соответствует', 'green')
    ...
    def st_test_20_bdu_1m(self) -> bool:
        ...
        if in_a2 is False:
            ...
            if in_a1 is False:
                self.mysql_conn.mysql_error(200)
            elif in_a2 is True:
                self.mysql_conn.mysql_error(201)
            return False
    ...
    def st_test_24_bdu_1m(self) -> bool:
        ...
        if in_a2 is False:
            ...
            self.fault.debug_msg('тест 2.4 положение выходов не соответствует', 'red')
            self.mysql_conn.mysql_ins_result("неисправен", '2')
            if in_a1 is False:
                self.mysql_conn.mysql_error(206)
            elif in_a2 is True:
                self.mysql_conn.mysql_error(207)
            return False
        self.fault.debug_msg('тест 2.4 положение выходов соответствует', 'green')
    ...
    def st_test_31_bdu_1m(self) -> bool:
        ...
        if in_a2 is True:
            ...
            if in_a1 is True:
                self.mysql_conn.mysql_error(208)
            elif in_a2 is False:
                self.mysql_conn.mysql_error(209)
            return False
    ...
    def st_test_41_bdu_1m(self) -> bool:
        ...
        if in_a2 is False:
            ...
            self.fault.debug_msg('тест 4.3 положение выходов не соответствует', 'red')
            self.mysql_conn.mysql_ins_result("неисправен", '4')
            if in_a1 is False:
                self.mysql_conn.mysql_error(210)
            elif in_a2 is True:
                self.mysql_conn.mysql_error(211)
            return False
        self.fault.debug_msg('тест 4.3 положение выходов соответствует', 'green')
        self.ctrl_kl.ctrl_relay('KL12', False)
        self.ctrl_kl.ctrl_relay('KL25', False)
    ...
    def st_test_51_bdu_1m(self) -> bool:
        ...
        if in_a2 is False:
            ...
            if in_a1 is False:
                self.mysql_conn.mysql_error(212)
            elif in_a2 is True:
                self.mysql_conn.mysql_error(213)
            return False
        self.fault.debug_msg('тест 5.2 положение выходов соответствует', 'green')
        self.ctrl_kl.ctrl_relay('KL12', False)
        self.ctrl_kl.ctrl_relay('KL1', False)
        self.ctrl_kl.ctrl_relay('KL25', False)
    ...
    def st_test_61_bdu_1m(self) -> bool:
        self.mysql_conn.mysql_ins_result("идёт тест 6.3", '6')
        self.ctrl_kl.ctrl_relay('KL12', False)
        sleep(2)
        in_a1, in_a2 = self.__inputs_a()
        if in_a2 is False:
            ...
            self.fault.debug_msg('тест 6.2 положение выходов не соответствует', 'red')
            self.mysql_conn.mysql_ins_result("неисправен", '6')
            if in_a1 is False:
                self.mysql_conn.mysql_error(214)
            elif in_a2 is True:
                self.mysql_conn.mysql_error(215)
            return False
        self.fault.debug_msg('тест 6.2 положение выходов соответствует', 'green')
    ...
    def __subtest_22(self, subtest_2_num: float, test_2_num: int) -> bool:
        """
            2.2. Включение блока от кнопки «Пуск»
        """
        self.mysql_conn.mysql_ins_result(f'идёт тест {subtest_2_num}', f'{test_2_num}')
        self.ctrl_kl.ctrl_relay('KL22', True)
        self.ctrl_kl.ctrl_relay('KL1', False)
        self.ctrl_kl.ctrl_relay('KL25', False)
        sleep(1)
        ...
        if in_a2 is True:
            ...
            self.mysql_conn.mysql_ins_result("неисправен", f'{test_2_num}')
            self.fault.debug_msg(f'тест {subtest_2_num} положение выходов не соответствует', 'red')
            if in_a1 is False:
                self.mysql_conn.mysql_error(202)
            elif in_a2 is False:
                self.mysql_conn.mysql_error(203)
            return False
        self.fault.debug_msg(f'тест {subtest_2_num} положение выходов соответствует', 'green')
        return True
    ...
    def __subtest_23(self, subtest_3_num: float, test_3_num: int) -> bool:
        """
            2.3. Проверка удержания блока во включенном состоянии при подключении Rш пульта управления:
        """
        self.mysql_conn.mysql_ins_result(f'идёт тест {subtest_3_num}', f'{test_3_num}')
        ...
        if in_a2 is True:
            ...
            self.mysql_conn.mysql_ins_result("неисправен", f'{test_3_num}')
            self.fault.debug_msg(f'тест {subtest_3_num} положение выходов не соответствует', 'red')
            if in_a1 is False:
                self.mysql_conn.mysql_error(204)
            elif in_a2 is False:
                self.mysql_conn.mysql_error(205)
            return False
        self.fault.debug_msg(f'тест {subtest_3_num} положение выходов соответствует', 'green')
        return True
